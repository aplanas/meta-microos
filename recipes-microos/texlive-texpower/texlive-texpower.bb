...
RPROVIDES:${PN} += "tex(automata.sty) tex(fixseminar.sty) tex(powersem.cls) tex(texpower.sty) tex(tpcolors.cfg) tex(tplists.sty) tex(tpoptions.cfg) tex(tppstcol.sty) tex(tpsem-a4.sty) tex(tpsettings.cfg) texlive-texpower"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(calc.sty) tex(color.sty) tex(graphics.sty) tex(ifpdf.sty) tex(ifthen.sty) tex(keyval.sty) tex(pstricks.sty) tex(relsize.sty) tex(sem-page.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin texlive-tpslifonts"
