...
RPROVIDES:${PN} += "tex(tcbbreakable.code.tex) tex(tcbdocumentation.code.tex) tex(tcbexternal.code.tex) tex(tcbfitting.code.tex) tex(tcbhooks.code.tex) tex(tcblistings.code.tex) tex(tcblistingscore.code.tex) tex(tcblistingsutf8.code.tex) tex(tcbmagazine.code.tex) tex(tcbminted.code.tex) tex(tcbposter.code.tex) tex(tcbprocessing.code.tex) tex(tcbraster.code.tex) tex(tcbskins.code.tex) tex(tcbskinsjigsaw.code.tex) tex(tcbtheorems.code.tex) tex(tcbvignette.code.tex) tex(tcbxparse.code.tex) tex(tcolorbox.sty) texlive-tcolorbox"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(environ.sty) tex(etoolbox.sty) tex(pgf.sty) tex(verbatim.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
