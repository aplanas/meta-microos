...
RPROVIDES:${PN} += "tex(Ufgeit.fd) tex(Ufgerm.fd) tex(fge.cfg) tex(fge.map) tex(fge.sty) tex(fgeit10.tfm) tex(fgerm10.tfm) texlive-fge"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(updmap.cfg) texlive texlive-fge-fonts texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
