...
RPROVIDES:${PN} += "tex(AlphaDia.tfm) tex(SkakNew-Diagram.tfm) tex(SkakNew-DiagramT.tfm) tex(SkakNew-Figurine.tfm) tex(SkakNew-FigurineBold.tfm) tex(SkakNew.map) texlive-skaknew"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(updmap.cfg) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin texlive-skaknew-fonts"
