...
RPROVIDES:${PN} += "group(audio) group(cdrom) group(dialout) group(disk) group(input) group(kmem) group(lock) group(lp) group(render) group(sgx) group(tape) group(tty) group(utmp) group(video) system-group-hardware"
