...
RDEPENDS:${PN} += "/bin/bash /bin/sh /usr/bin/clear /usr/bin/dialog /usr/bin/mkdir /usr/bin/mktemp /usr/bin/mv /usr/bin/perl /usr/bin/setpriv /usr/bin/stat /usr/bin/touch coreutils ed fillup findutils grep group(mktex) permissions python3 rpm_macro(_unitdir) rpm_macro(service_add_post) rpm_macro(service_add_pre) rpm_macro(service_del_postun) rpm_macro(service_del_preun) sed texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin user(mktex)"
