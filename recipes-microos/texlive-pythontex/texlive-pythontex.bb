...
RPROVIDES:${PN} += "tex(pythontex.sty) texlive-pythontex"
RDEPENDS:${PN} += "/bin/sh /usr/bin/python3 coreutils ed findutils grep sed tex(currfile.sty) tex(etoolbox.sty) tex(fvextra.sty) tex(newfloat.sty) tex(pgfopts.sty) tex(upquote.sty) tex(xcolor.sty) tex(xstring.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-pythontex-bin texlive-scripts texlive-scripts-bin"
