...
RPROVIDES:${PN} += "tex(bookshelf.cls) texlive-bookshelf"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(calc.sty) tex(eso-pic.sty) tex(fix-cm.sty) tex(fontspec.sty) tex(fp.sty) tex(graphicx.sty) tex(random.tex) tex(report.cls) tex(xcolor.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
