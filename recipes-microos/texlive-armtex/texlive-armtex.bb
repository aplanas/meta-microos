...
RPROVIDES:${PN} += "tex(arm.tex) tex(armkb-a8.tex) tex(armkb-u8.tex) tex(armscii8.def) tex(armtex.sty) tex(arss.map) tex(arssb10.tfm) tex(arssbs10.tfm) tex(arssr10.tfm) tex(arsssl10.tfm) tex(artm.map) tex(artmb10.tfm) tex(artmbi10.tfm) tex(artmbs10.tfm) tex(artmi10.tfm) tex(artmr10.tfm) tex(artmsl10.tfm) tex(ot6cmr.fd) tex(ot6cmss.fd) tex(ot6enc.def) texlive-armtex"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(fontenc.sty) tex(kvoptions.sty) tex(updmap.cfg) texlive texlive-armtex-fonts texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
