...
RPROVIDES:${PN} += "tex(amsart.cls) tex(amsbook.cls) tex(amsbooka.sty) tex(amsdtx.cls) tex(amsldoc.cls) tex(amsmidx.sty) tex(amsproc.cls) tex(amsthm.sty) tex(upref.sty) texlive-amscls"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(amsfonts.sty) tex(book.cls) tex(doc.sty) tex(url.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
