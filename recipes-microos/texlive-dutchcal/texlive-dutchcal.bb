...
RPROVIDES:${PN} += "tex(dutchcal-b.tfm) tex(dutchcal-b.vf) tex(dutchcal-r.tfm) tex(dutchcal-r.vf) tex(dutchcal.map) tex(dutchcal.sty) tex(rdutchcalb.tfm) tex(rdutchcalr.tfm) tex(udutchcal.fd) texlive-dutchcal"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(cmr10.tfm) tex(updmap.cfg) tex(xkeyval.sty) texlive texlive-dutchcal-fonts texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
