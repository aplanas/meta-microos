...
RPROVIDES:${PN} += "tex(bankstatement.cls) tex(csv-camt.def) tex(csv-mt940.def) tex(csv-standard-bank-na.def) tex(stmenglish.def) tex(stmgerman.def) tex(stmnamibian.def) texlive-bankstatement"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(article.cls) tex(booktabs.sty) tex(calc.sty) tex(datatool.sty) tex(graphicx.sty) tex(ifthen.sty) tex(longtable.sty) tex(siunitx.sty) tex(tabularx.sty) tex(xcolor.sty) tex(xkeyval.sty) tex(xkvltxp.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
