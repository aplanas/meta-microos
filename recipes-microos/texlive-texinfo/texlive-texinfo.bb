...
RPROVIDES:${PN} += "tex(texinfo.tex) tex(txi-cs.tex) tex(txi-de.tex) tex(txi-en.tex) tex(txi-es.tex) tex(txi-fr.tex) tex(txi-it.tex) tex(txi-nb.tex) tex(txi-nl.tex) tex(txi-nn.tex) tex(txi-pl.tex) tex(txi-pt.tex) tex(txi-ru.tex) tex(txi-sr.tex) tex(txi-tr.tex) tex(txi-uk.tex) texlive-texinfo"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed texlive texlive-amsfonts texlive-cm-super texlive-ec texlive-epsf texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
