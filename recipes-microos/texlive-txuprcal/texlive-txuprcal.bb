...
RPROVIDES:${PN} += "tex(TXUprCal.map) tex(txUprCal-Bold.tfm) tex(txUprCal-Regular.tfm) tex(txuprcal.sty) tex(utxuprcal.fd) texlive-txuprcal"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(updmap.cfg) tex(xkeyval.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin texlive-txuprcal-fonts"
