...
RPROVIDES:${PN} += "tex(chemschemex.sty) texlive-chemschemex"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(etoolbox.sty) tex(fancylabel.sty) tex(graphicx.sty) tex(suffix.sty) tex(tikz.sty) tex(xargs.sty) tex(xifthen.sty) tex(xkeyval.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
