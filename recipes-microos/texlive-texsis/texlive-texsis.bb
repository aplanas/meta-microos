...
RPROVIDES:${PN} += "tex(TXSconts.tex) tex(TXSdcol.tex) tex(TXSenvmt.tex) tex(TXSeqns.tex) tex(TXSfigs.tex) tex(TXSfmts.tex) tex(TXSfonts.tex) tex(TXShead.tex) tex(TXSinit.tex) tex(TXSletr.tex) tex(TXSmacs.tex) tex(TXSmemo.tex) tex(TXSprns.tex) tex(TXSrefs.tex) tex(TXSruled.tex) tex(TXSsects.tex) tex(TXSsite.tex) tex(TXSsymb.tex) tex(TXStags.tex) tex(TXStitle.tex) tex(texsis.tex) texlive-texsis"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(language.dat) tex(language.dat.lua) tex(language.def) texlive texlive-cm texlive-filesystem texlive-hyphen-base texlive-knuth-lib texlive-kpathsea texlive-kpathsea-bin texlive-plain texlive-scripts texlive-scripts-bin texlive-tex texlive-texsis-bin"
