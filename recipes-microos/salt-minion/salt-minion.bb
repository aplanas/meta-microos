...
RPROVIDES:${PN} += "config(salt-minion) salt-minion salt-minion(aarch-64)"
RDEPENDS:${PN} += "(salt-transactional-update = 3006.0-1.1 if read-only-root-fs) /bin/sh /usr/bin/mkdir /usr/bin/python3 /usr/bin/touch fillup salt systemd"
