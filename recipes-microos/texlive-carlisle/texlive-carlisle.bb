...
RPROVIDES:${PN} += "tex(dotlessj.sty) tex(ltxtable.sty) tex(plain.sty) tex(remreset.sty) tex(scalefnt.sty) tex(slashed.sty) texlive-carlisle"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(color.sty) tex(longtable.sty) tex(tabularx.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
