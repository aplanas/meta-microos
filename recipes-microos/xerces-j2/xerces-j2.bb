...
RPROVIDES:${PN} += "jaxp_parser_impl mvn(apache:xerces-j2) mvn(apache:xerces-j2:pom:) mvn(xerces:xerces) mvn(xerces:xerces:pom:) mvn(xerces:xercesImpl) mvn(xerces:xercesImpl:pom:) mvn(xerces:xmlParserAPIs) mvn(xerces:xmlParserAPIs:pom:) osgi(org.apache.xerces) xerces-j2 xerces-j2-scripts"
RDEPENDS:${PN} += "/bin/sh java-headless javapackages-filesystem javapackages-tools mvn(xml-apis:xml-apis) osgi(javax.xml) osgi(org.apache.xml.resolver) osgi(org.apache.xml.serializer) xalan-j2 xml-commons-apis xml-commons-resolver"
