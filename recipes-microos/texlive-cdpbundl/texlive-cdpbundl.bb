...
RPROVIDES:${PN} += "tex(articoletteracdp.cls) tex(cdpaddon.sty) tex(cdpbabel.sty) tex(cdpnamesenglish.ldf) tex(cdpnamesitalian.ldf) tex(cdpshues-example.def) tex(cdpshues.cfg) tex(epson-stylus-740.def) tex(hp-laserjet-4500.def) tex(letteracdp.cls) texlive-cdpbundl"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(babel.sty) tex(color.sty) tex(eepic.sty) tex(hyperref.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
