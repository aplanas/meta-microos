...
RPROVIDES:${PN} += "typelib(MalcontentUi) typelib-1_0-MalcontentUi-0 typelib-1_0-MalcontentUi-0(aarch-64)"
RDEPENDS:${PN} += "libmalcontent-ui-0.so.0()(64bit) typelib(AccountsService) typelib(Atk) typelib(GLib) typelib(GModule) typelib(GObject) typelib(Gdk) typelib(GdkPixbuf) typelib(Gio) typelib(Gtk) typelib(HarfBuzz) typelib(Malcontent) typelib(Pango) typelib(cairo) typelib(freetype2) typelib(xlib)"
