...
RPROVIDES:${PN} += "typelib(GnomeAutoarGtk) typelib-1_0-GnomeAutoarGtk-0_1 typelib-1_0-GnomeAutoarGtk-0_1(aarch-64)"
RDEPENDS:${PN} += "libgnome-autoar-gtk-0.so.0()(64bit) typelib(Atk) typelib(GLib) typelib(GModule) typelib(GObject) typelib(Gdk) typelib(GdkPixbuf) typelib(Gio) typelib(GnomeAutoar) typelib(Gtk) typelib(HarfBuzz) typelib(Pango) typelib(cairo) typelib(freetype2) typelib(xlib)"
