...
RPROVIDES:${PN} += "tex(8ruzc.fd) tex(omluzc.fd) tex(omsuzc.fd) tex(ot1uzc.fd) tex(pzcmi.tfm) tex(pzcmi.vf) tex(pzcmi7t.tfm) tex(pzcmi7t.vf) tex(pzcmi8c.tfm) tex(pzcmi8c.vf) tex(pzcmi8r.tfm) tex(pzcmi8t.tfm) tex(pzcmi8t.vf) tex(t1uzc.fd) tex(ts1uzc.fd) tex(uzc.map) tex(uzcmi7t.tfm) tex(uzcmi7t.vf) tex(uzcmi8c.tfm) tex(uzcmi8c.vf) tex(uzcmi8r.tfm) tex(uzcmi8t.tfm) tex(uzcmi8t.vf) texlive-zapfchan"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(updmap.cfg) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin texlive-zapfchan-fonts"
