...
RPROVIDES:${PN} += "application() application(redshift-gtk.desktop) metainfo() metainfo(redshift-gtk.appdata.xml) redshift-gtk redshift-gtk(aarch-64)"
RDEPENDS:${PN} += "/bin/sh /usr/bin/python3 hicolor-icon-theme python(abi) python3-gobject-Gdk redshift systemd typelib(AppIndicator3) typelib(GLib) typelib(GObject) typelib(Gtk)"
