...
RPROVIDES:${PN} += "application() application(ccsm.desktop) ccsm compizconfig-settings-manager metainfo() metainfo(ccsm.appdata.xml)"
RDEPENDS:${PN} += "/usr/bin/python3 python3-ccm typelib(GLib) typelib(Gdk) typelib(Gio) typelib(Gtk) typelib(Pango) typelib(PangoCairo)"
