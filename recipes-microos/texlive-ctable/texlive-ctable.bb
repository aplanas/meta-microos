...
RPROVIDES:${PN} += "tex(ctable.sty) texlive-ctable"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(array.sty) tex(booktabs.sty) tex(etoolbox.sty) tex(ifpdf.sty) tex(rotating.sty) tex(tabularx.sty) tex(transparent.sty) tex(xcolor.sty) tex(xkeyval.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
