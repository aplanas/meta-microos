...
RPROVIDES:${PN} += "tex(gfdl-tex-1p1.tex) tex(gfdl-tex-1p2.tex) tex(gfdl-tex-1p3.tex) tex(gfdl.sty) texlive-gfdl"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(csquotes.sty) tex(expkv-def.sty) tex(expkv-opt.sty) tex(float.sty) tex(hyperref.sty) tex(hyperxmp.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
