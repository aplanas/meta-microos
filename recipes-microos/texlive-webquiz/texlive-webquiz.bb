...
RPROVIDES:${PN} += "tex(webquiz-doc.code.tex) tex(webquiz-ini.code.tex) tex(webquiz.cfg) tex(webquiz.cls) texlive-webquiz"
RDEPENDS:${PN} += "/bin/sh /usr/bin/python3 coreutils ed findutils grep sed tex(amsfonts.sty) tex(amsmath.sty) tex(article.cls) tex(bbding.sty) tex(etoolbox.sty) tex(pgffor.sty) tex(pgfopts.sty) tex(tikz.sty) tex(xparse.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin texlive-webquiz-bin"
