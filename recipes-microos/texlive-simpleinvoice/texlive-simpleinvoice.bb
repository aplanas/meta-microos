...
RPROVIDES:${PN} += "tex(simpleinvoice.sty) texlive-simpleinvoice"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(advdate.sty) tex(babel.sty) tex(colortbl.sty) tex(etoolbox.sty) tex(hyperref.sty) tex(tabularx.sty) tex(url.sty) tex(xcolor.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
