...
RDEPENDS:${PN} += "/usr/bin/perl perl(:MODULE_COMPAT_5.36.0) perl(Encode::Locale) perl(File::Listing) perl(HTML::Entities) perl(HTML::HeadParser) perl(HTTP::CookieJar::LWP) perl(HTTP::Cookies) perl(HTTP::Date) perl(HTTP::Negotiate) perl(HTTP::Request) perl(HTTP::Request::Common) perl(HTTP::Response) perl(HTTP::Status) perl(LWP::MediaTypes) perl(Module::Load) perl(Net::HTTP) perl(Try::Tiny) perl(URI) perl(URI::Escape) perl(WWW::RobotRules) perl(parent)"
