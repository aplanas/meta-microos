...
RPROVIDES:${PN} += "tex(legislation.cls) texlive-umbclegislation"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(article.cls) tex(draftwatermark.sty) tex(enumitem.sty) tex(etoolbox.sty) tex(geometry.sty) tex(lineno.sty) tex(mdframed.sty) tex(xifthen.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
