...
RPROVIDES:${PN} += "tex(aliasctr.sty) tex(parseargs.sty) tex(thm-amsthm.sty) tex(thm-autoref.sty) tex(thm-beamer.sty) tex(thm-kv.sty) tex(thm-listof.sty) tex(thm-llncs.sty) tex(thm-ntheorem.sty) tex(thm-patch.sty) tex(thm-restate.sty) tex(thmdef-mdframed.sty) tex(thmdef-shaded.sty) tex(thmdef-thmbox.sty) tex(thmtools.sty) tex(unique.sty) texlive-thmtools"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(etex.sty) tex(keyval.sty) tex(kvsetkeys.sty) tex(mdframed.sty) tex(remreset.sty) tex(shadethm.sty) tex(thmbox.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
