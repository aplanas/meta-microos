...
RPROVIDES:${PN} += "tex(invoice-class.cls) texlive-invoice-class"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(array.sty) tex(article.cls) tex(datatool.sty) tex(dcolumn.sty) tex(fancyhdr.sty) tex(geometry.sty) tex(longtable.sty) tex(multicol.sty) tex(tabularx.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
