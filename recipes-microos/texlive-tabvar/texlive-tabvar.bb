...
RPROVIDES:${PN} += "tex(tabvar.cfg) tex(tabvar.map) tex(tabvar.sty) tex(tabvar.tfm) texlive-tabvar"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(array.sty) tex(colortbl.sty) tex(graphicx.sty) tex(ifpdf.sty) tex(ifthen.sty) tex(updmap.cfg) tex(varwidth.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin texlive-tabvar-fonts"
