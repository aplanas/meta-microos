...
RPROVIDES:${PN} += "tex(BBOLDX-Bold.tfm) tex(BBOLDX-Regular.tfm) tex(BBOLDX-Thin.tfm) tex(Ubboldx.fd) tex(bboldx.enc) tex(bboldx.map) tex(bboldx.sty) texlive-bboldx"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(updmap.cfg) tex(xkeyval.sty) texlive texlive-bboldx-fonts texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
