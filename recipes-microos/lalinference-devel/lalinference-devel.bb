...
RPROVIDES:${PN} += "config(lalinference-devel) lalinference-devel lalinference-devel(aarch-64) pkgconfig(lalinference)"
RDEPENDS:${PN} += "/usr/bin/pkg-config lalinference-data liblalinference23 pkgconfig(gsl) pkgconfig(lal) pkgconfig(lalburst) pkgconfig(lalframe) pkgconfig(lalinspiral) pkgconfig(lalmetaio) pkgconfig(lalsimulation)"
