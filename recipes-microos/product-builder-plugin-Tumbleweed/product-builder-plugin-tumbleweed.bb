...
RPROVIDES:${PN} += "config(product-builder-plugin-Tumbleweed) perl(KIWIBasePlugin) perl(KIWIBootIsoPlugin) perl(KIWIChecksumPlugin) perl(KIWIDescrPlugin) perl(KIWIDnfRepoclosurePlugin) perl(KIWIEulaPlugin) perl(KIWIInstallCheckPlugin) perl(KIWIMiniIsoPlugin) perl(KIWIPackagesDVDPlugin) product-builder-plugin product-builder-plugin-Tumbleweed"
RDEPENDS:${PN} += "build createrepo_c dnf instsource-susedata mkdosfs mtools openSUSE-appstream-process package-EULAs package-translations perl(YAML::XS) product-builder"
