...
RPROVIDES:${PN} += "tex(basic.def) tex(box.def) tex(cioptic.tfm) tex(circ.sty) tex(gate.def) tex(ic.def) tex(oldgate.def) tex(optics.def) tex(physics.def) texlive-circ"
