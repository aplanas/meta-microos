...
RPROVIDES:${PN} += "libspice-server-devel libspice-server-devel(aarch-64) pkgconfig(spice-server)"
RDEPENDS:${PN} += "/usr/bin/pkg-config libspice-server1 pkgconfig(glib-2.0) pkgconfig(libcacard) pkgconfig(openssl) pkgconfig(pixman-1) pkgconfig(spice-protocol)"
