...
RPROVIDES:${PN} += "tex(mycv.cls) tex(mycv_base.def) tex(mycv_dec.sty) tex(mycv_misc.def) tex(mycv_style.sty) tex(mycv_version.def) texlive-mycv"
RDEPENDS:${PN} += "/bin/sh /usr/bin/perl coreutils ed findutils grep perl(Cwd) perl(File::Path) perl(Getopt::Long) perl(strict) perl(warnings) sed tex(amssymb.sty) tex(article.cls) tex(babel.sty) tex(calligra.sty) tex(etextools.sty) tex(fancyhdr.sty) tex(hyperref.sty) tex(ifthen.sty) tex(kvoptions.sty) tex(marvosym.sty) tex(pifont.sty) tex(svn-prov.sty) tex(tikz.sty) tex(times.sty) tex(titlesec.sty) tex(xcolor.sty) tex(xparse.sty) tex(xstring.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
