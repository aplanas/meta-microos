...
RPROVIDES:${PN} += "tex(fancyhandout.cls) texlive-fancyhandout"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(article.cls) tex(csquotes.sty) tex(enumitem.sty) tex(etoolbox.sty) tex(fancyhdr.sty) tex(geometry.sty) tex(xcolor.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
