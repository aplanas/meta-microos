...
RPROVIDES:${PN} += "tex(beuron.map) tex(beuron.sty) tex(beuron.tfm) tex(beuronc.tfm) tex(beuronx.tfm) tex(t1beuron.fd) texlive-beuron"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(expl3.sty) tex(fontspec.sty) tex(updmap.cfg) tex(xparse.sty) texlive texlive-beuron-fonts texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
