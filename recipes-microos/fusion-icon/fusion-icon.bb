...
RPROVIDES:${PN} += "application() application(fusion-icon.desktop) fusion-icon metainfo() metainfo(fusion-icon.appdata.xml) python3.10dist(fusion-icon) python3dist(fusion-icon)"
RDEPENDS:${PN} += "/bin/sh /usr/bin/python3 Mesa-demo-x python(abi) python3-compizconfig python3-gobject python3-gobject-Gdk python3-qt5 typelib(AppIndicator3) typelib(Gdk) typelib(Gtk) xvinfo"
