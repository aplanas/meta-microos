...
RPROVIDES:${PN} += "tex(pdfpages.sty) tex(ppdvipdfmx.def) tex(ppdvips.def) tex(ppluatex.def) tex(ppnull.def) tex(pppdftex.def) tex(ppvtex.def) tex(ppxetex.def) texlive-pdfpages"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(calc.sty) tex(count1to.sty) tex(dvipdfmx.def) tex(eso-pic.sty) tex(graphicx.sty) tex(ifthen.sty) tex(pdflscape.sty) texlive texlive-eso-pic texlive-filesystem texlive-graphics texlive-kpathsea texlive-kpathsea-bin texlive-oberdiek texlive-scripts texlive-scripts-bin texlive-tools"
