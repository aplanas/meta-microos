...
RPROVIDES:${PN} += "perl(Benchmark::Dumb) perl(Dumbbench) perl(Dumbbench::BoxPlot) perl(Dumbbench::CPUFrequencyPinner) perl(Dumbbench::Instance) perl(Dumbbench::Instance::Cmd) perl(Dumbbench::Instance::PerlEval) perl(Dumbbench::Instance::PerlSub) perl(Dumbbench::Result) perl(Dumbbench::Stats) perl-Dumbbench"
RDEPENDS:${PN} += "/usr/bin/perl perl(:MODULE_COMPAT_5.36.0) perl(Capture::Tiny) perl(Class::XSAccessor) perl(Devel::CheckOS) perl(Number::WithError) perl(Params::Util) perl(Statistics::CaseResampling) perl(parent)"
