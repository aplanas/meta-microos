...
RPROVIDES:${PN} += "tex(listings.cfg) tex(listings.sty) tex(lstdoc.sty) tex(lstlang1.sty) tex(lstlang2.sty) tex(lstlang3.sty) tex(lstmisc.sty) texlive-listings"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(algorithmic.sty) tex(color.sty) tex(fancyvrb.sty) tex(hyperref.sty) tex(keyval.sty) tex(nameref.sty) tex(textcomp.sty) tex(upquote.sty) tex(url.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
