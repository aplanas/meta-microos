...
RPROVIDES:${PN} += "tex(pst-uml.sty) texlive-pst-uml"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(calc.sty) tex(graphicx.sty) tex(ifthen.sty) tex(multido.sty) tex(pst-node.sty) tex(pst-tree.sty) tex(pst-xkey.sty) tex(pstricks.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-multido texlive-scripts texlive-scripts-bin"
