...
RPROVIDES:${PN} += "tex(arabi-add.sty) texlive-arabi-add"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(amsthm.sty) tex(atveryend.sty) tex(bookmark.sty) tex(datatool.sty) tex(hyperref.sty) tex(ifthen.sty) tex(xcolor.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
