...
RPROVIDES:${PN} += "libsigrok-devel libsigrok-devel(aarch-64) pkgconfig(libsigrok) pkgconfig(libsigrokcxx)"
RDEPENDS:${PN} += "/usr/bin/pkg-config libsigrok4 libsigrokcxx4 pkgconfig(bluez) pkgconfig(gio-2.0) pkgconfig(glib-2.0) pkgconfig(glibmm-2.4) pkgconfig(hidapi-hidraw) pkgconfig(libftdi1) pkgconfig(libserialport) pkgconfig(libsigrok) pkgconfig(libtirpc) pkgconfig(libusb-1.0) pkgconfig(libzip)"
