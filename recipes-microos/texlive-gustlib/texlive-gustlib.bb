...
RPROVIDES:${PN} += "tex(biblotex.tex) tex(infr-ex.tex) tex(infram.tex) tex(map.tex) tex(mcol-ex.tex) tex(meashor.tex) tex(mimulcol.tex) tex(plidxmac.tex) tex(przyklad.tex) tex(rbox-ex.tex) tex(roundbox.tex) tex(split.tex) tex(tp-crf.tex) tex(tsp.tex) tex(tun.tex) tex(verbatim-dek.tex) texlive-gustlib"
