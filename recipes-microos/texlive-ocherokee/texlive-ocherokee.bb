...
RPROVIDES:${PN} += "tex(Cherokee.tfm) tex(Cherokeeb.tfm) tex(Cherokeebo.tfm) tex(Cherokeeo.tfm) tex(cherokee.map) tex(lchcmr.fd) tex(lchenc.def) tex(ocherokee.sty) texlive-ocherokee"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(updmap.cfg) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-ocherokee-fonts texlive-scripts texlive-scripts-bin"
