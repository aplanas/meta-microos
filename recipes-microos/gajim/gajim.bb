...
RPROVIDES:${PN} += "application() application(org.gajim.Gajim.desktop) gajim metainfo() metainfo(org.gajim.Gajim.appdata.xml) mimehandler(x-scheme-handler/xmpp) python3.10dist(gajim) python3dist(gajim)"
RDEPENDS:${PN} += "(python3.10dist(nbxmpp) < 5 with python3.10dist(nbxmpp) >= 4.2.2) /usr/bin/python3.10 ca-certificates-mozilla python(abi) python3.10dist(cryptography) python3.10dist(css-parser) python3.10dist(gssapi) python3.10dist(keyring) python3.10dist(packaging) python3.10dist(pillow) python3.10dist(precis-i18n) python3.10dist(pycairo) python3.10dist(pygobject) python310-gobject-Gdk python310-gobject-cairo typelib(AyatanaAppIndicator3) typelib(Farstream) typelib(GLib) typelib(GObject) typelib(GSound) typelib(Gdk) typelib(GdkPixbuf) typelib(Geoclue) typelib(Gio) typelib(Gspell) typelib(Gst) typelib(GstPbutils) typelib(Gtk) typelib(GtkSource) typelib(Pango) typelib(PangoCairo) typelib(Soup)"
