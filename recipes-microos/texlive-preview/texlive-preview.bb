...
RPROVIDES:${PN} += "tex(prauctex.cfg) tex(prauctex.def) tex(prcounters.def) tex(preview.sty) tex(prfootnotes.def) tex(prlyx.def) tex(prshowbox.def) tex(prshowlabels.def) tex(prtightpage.def) tex(prtracingall.def) texlive-preview"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(luatex85.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
