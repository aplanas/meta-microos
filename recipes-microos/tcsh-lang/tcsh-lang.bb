...
RPROVIDES:${PN} += "locale(tcsh:de) locale(tcsh:el) locale(tcsh:el_GR) locale(tcsh:es) locale(tcsh:et) locale(tcsh:fi) locale(tcsh:fr) locale(tcsh:it) locale(tcsh:ja) locale(tcsh:pl) locale(tcsh:ru_RU) locale(tcsh:ru_UA.koi8u) locale(tcsh:uk_UA) tcsh-lang tcsh-lang-all"
RDEPENDS:${PN} += "tcsh"
