...
RPROVIDES:${PN} += "tex(pbsi.map) tex(pbsi.sty) tex(pbsi.tfm) tex(pbsi8r.tfm) tex(pbsi8t.tfm) tex(pbsi8t.vf) tex(t1pbsi.fd) texlive-brushscr"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(updmap.cfg) texlive texlive-brushscr-fonts texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
