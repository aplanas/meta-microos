...
RPROVIDES:${PN} += "tex(OrnementsADF.enc) tex(OrnementsADF.map) tex(OrnementsADF.tfm) tex(adforn.sty) tex(uornementsadf.fd) texlive-adforn"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(pifont.sty) tex(updmap.cfg) texlive texlive-adforn-fonts texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
