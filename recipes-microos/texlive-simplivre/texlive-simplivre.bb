...
RPROVIDES:${PN} += "tex(simplivre.cls) texlive-simplivre"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(amssymb.sty) tex(caption.sty) tex(ctex.sty) tex(draftwatermark.sty) tex(embrac.sty) tex(float.sty) tex(fontspec.sty) tex(geometry.sty) tex(graphicx.sty) tex(indentfirst.sty) tex(lua-widow-control.sty) tex(mathpazo.sty) tex(minimalist.sty) tex(nowidow.sty) tex(parskip.sty) tex(projlib-font.sty) tex(regexpatch.sty) tex(silence.sty) tex(tikz-cd.sty) tex(unicode-math.sty) tex(wrapfig.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-minimalist texlive-scripts texlive-scripts-bin"
