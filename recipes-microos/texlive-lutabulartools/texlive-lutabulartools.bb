...
RPROVIDES:${PN} += "tex(lutabulartools.sty) texlive-lutabulartools"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(array.sty) tex(booktabs.sty) tex(colortbl.sty) tex(luacode.sty) tex(makecell.sty) tex(multirow.sty) tex(penlight.sty) tex(xcolor.sty) tex(xparse.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
