...
RPROVIDES:${PN} += "application() application(org.gnome.GnomeSubtitles.desktop) gnome-subtitles gnome-subtitles(aarch-64) libgst_backend.so()(64bit) metainfo() metainfo(org.gnome.GnomeSubtitles.appdata.xml) mimehandler(application/x-subrip) mimehandler(text/vtt) mimehandler(text/x-microdvd) mimehandler(text/x-mpsub) mimehandler(text/x-ssa) mimehandler(text/x-subviewer) mono(gnome-subtitles)"
RDEPENDS:${PN} += "/bin/bash gstreamer-plugins-base hicolor-icon-theme ld-linux-aarch64.so.1()(64bit) ld-linux-aarch64.so.1(GLIBC_2.17)(64bit) libc.so.6(GLIBC_2.17)(64bit) libglib-2.0.so.0()(64bit) libgobject-2.0.so.0()(64bit) libgstpbutils-1.0.so.0()(64bit) libgstreamer-1.0.so.0()(64bit) mono(Mono.Posix) mono(System) mono(cairo-sharp) mono(gdk-sharp) mono(gio-sharp) mono(glib-sharp) mono(gtk-sharp) mono(mscorlib) mono(pango-sharp) mono-core"
