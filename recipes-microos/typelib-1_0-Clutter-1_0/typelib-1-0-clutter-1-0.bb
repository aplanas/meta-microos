...
RPROVIDES:${PN} += "typelib(Cally) typelib(Clutter) typelib(ClutterGdk) typelib(ClutterX11) typelib-1_0-Clutter-1_0 typelib-1_0-Clutter-1_0(aarch-64)"
RDEPENDS:${PN} += "libclutter-1.0.so.0()(64bit) typelib(Atk) typelib(Cogl) typelib(CoglPango) typelib(GL) typelib(GLib) typelib(GModule) typelib(GObject) typelib(Gdk) typelib(GdkPixbuf) typelib(Gio) typelib(HarfBuzz) typelib(Json) typelib(Pango) typelib(PangoCairo) typelib(cairo) typelib(freetype2) typelib(xlib)"
