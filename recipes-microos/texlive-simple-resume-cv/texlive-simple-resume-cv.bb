...
RPROVIDES:${PN} += "tex(simpleresumecv.cls) texlive-simple-resume-cv"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(article.cls) tex(color.sty) tex(datetime2.sty) tex(fontspec.sty) tex(hyperref.sty) tex(hyphenat.sty) tex(xltxtra.sty) tex(xunicode.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
