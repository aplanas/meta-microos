...
RPROVIDES:${PN} += "config(texlive-dvipdfmx) tex(cid-x.map) tex(ckx.map) tex(dvipdfmx-unsafe.cfg) tex(dvipdfmx.cfg) texlive-dvipdfm texlive-dvipdfmx"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils ghostscript grep perl perl(TeXLive::TLUtils) sed tex(texmf.cnf) texlive texlive-dvipdfmx-bin texlive-filesystem texlive-glyphlist texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin texlive-texlive.infra"
