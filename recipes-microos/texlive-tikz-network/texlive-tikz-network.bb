...
RPROVIDES:${PN} += "tex(tikz-network.sty) texlive-tikz-network"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(datatool.sty) tex(etex.sty) tex(graphicx.sty) tex(tikz.sty) tex(trimspaces.sty) tex(xifthen.sty) tex(xkeyval.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
