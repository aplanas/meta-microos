...
RPROVIDES:${PN} += "tex(rubikcube.sty) tex(rubikpatterns.sty) tex(rubikrotation.sty) tex(rubiktwocube.sty) texlive-rubik"
RDEPENDS:${PN} += "/bin/sh /usr/bin/perl coreutils ed findutils grep perl(Carp) perl(Fatal) perl(warnings) sed tex(fancyvrb.sty) tex(forarray.sty) tex(ifluatex.sty) tex(ifthen.sty) tex(shellesc.sty) tex(tikz.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-rubik-bin texlive-scripts texlive-scripts-bin"
