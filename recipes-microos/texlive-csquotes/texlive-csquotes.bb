...
RPROVIDES:${PN} += "tex(csq-compat.def) tex(csquotes.cfg) tex(csquotes.def) tex(csquotes.sty) texlive-csquotes"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(etoolbox.sty) tex(inputenc.sty) tex(keyval.sty) texlive texlive-etoolbox texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
