...
RPROVIDES:${PN} += "python3.9dist(tweepy) python39-tweepy python3dist(tweepy)"
RDEPENDS:${PN} += "(python3.9dist(oauthlib) < 4 with python3.9dist(oauthlib) >= 3.2) (python3.9dist(requests) < 3 with python3.9dist(requests) >= 2.27) (python3.9dist(requests-oauthlib) < 2 with python3.9dist(requests-oauthlib) >= 1.2) python(abi)"
