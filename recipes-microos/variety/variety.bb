...
RPROVIDES:${PN} += "application() application(variety.desktop) metainfo() metainfo(variety.appdata.xml) mimehandler(text/uri-list) python3.10dist(variety) python3dist(variety) variety"
RDEPENDS:${PN} += "/bin/bash /usr/bin/python3 ImageMagick libnotify4 python(abi) python3-Pillow python3-beautifulsoup4 python3-configobj python3-dbus-python python3-gexiv2 python3-gobject-Gdk python3-httplib2 python3-lxml python3-pycairo python3-pycurl python3-requests typelib(AppIndicator3) typelib(GExiv2) typelib(GLib) typelib(GObject) typelib(Gdk) typelib(GdkPixbuf) typelib(Gio) typelib(Gtk) typelib(Notify) typelib(Pango) typelib(PangoCairo) yelp"
