...
RPROVIDES:${PN} += "tex(LinearA.tfm) tex(LinearACmplxSigns.tfm) tex(linearA.map) tex(linearA.sty) texlive-lineara"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(updmap.cfg) tex(xspace.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-lineara-fonts texlive-scripts texlive-scripts-bin"
