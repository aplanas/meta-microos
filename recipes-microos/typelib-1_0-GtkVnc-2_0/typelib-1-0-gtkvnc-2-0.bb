...
RPROVIDES:${PN} += "typelib(GtkVnc) typelib-1_0-GtkVnc-2_0 typelib-1_0-GtkVnc-2_0(aarch-64)"
RDEPENDS:${PN} += "libgtk-vnc-2.0.so.0()(64bit) typelib(Atk) typelib(GLib) typelib(GModule) typelib(GObject) typelib(GVnc) typelib(Gdk) typelib(GdkPixbuf) typelib(Gio) typelib(Gtk) typelib(HarfBuzz) typelib(Pango) typelib(cairo) typelib(freetype2) typelib(xlib)"
