...
RPROVIDES:${PN} += "tex(svn-multi.sty) tex(svnkw.sty) texlive-svn-multi"
RDEPENDS:${PN} += "/bin/sh /usr/bin/perl coreutils ed findutils grep perl(Date::Format) perl(Date::Parse) perl(File::Basename) perl(strict) perl(warnings) sed tex(currfile.sty) tex(filehook.sty) tex(graphics.sty) tex(kvoptions.sty) tex(pgf.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin texlive-svn-multi-bin"
