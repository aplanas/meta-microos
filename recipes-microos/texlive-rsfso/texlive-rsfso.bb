...
RPROVIDES:${PN} += "tex(rrsfso10.tfm) tex(rrsfso5.tfm) tex(rrsfso7.tfm) tex(rsfso.map) tex(rsfso.sty) tex(rsfso10.tfm) tex(rsfso10.vf) tex(rsfso5.tfm) tex(rsfso5.vf) tex(rsfso7.tfm) tex(rsfso7.vf) tex(ursfso.fd) texlive-rsfso"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(cmr10.tfm) tex(cmr5.tfm) tex(cmr7.tfm) tex(updmap.cfg) tex(xkeyval.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
