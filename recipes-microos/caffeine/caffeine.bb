...
RPROVIDES:${PN} += "application() application(caffeine-indicator.desktop) application(caffeine.desktop) caffeine caffeine-lang python3.10dist(caffeine) python3dist(caffeine)"
RDEPENDS:${PN} += "/bin/sh /usr/bin/perl /usr/bin/python3 python(abi) python3-xdg python3-xlib typelib(AppIndicator3) typelib(GLib) typelib(GObject) typelib(Gtk)"
