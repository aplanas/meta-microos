...
RPROVIDES:${PN} += "application() application(feh.desktop) feh feh(aarch-64) mimehandler(image/bmp) mimehandler(image/gif) mimehandler(image/heic) mimehandler(image/jpeg) mimehandler(image/jpg) mimehandler(image/jxl) mimehandler(image/pjpeg) mimehandler(image/png) mimehandler(image/tiff) mimehandler(image/webp) mimehandler(image/x-bmp) mimehandler(image/x-pcx) mimehandler(image/x-png) mimehandler(image/x-portable-anymap) mimehandler(image/x-portable-bitmap) mimehandler(image/x-portable-graymap) mimehandler(image/x-portable-pixmap) mimehandler(image/x-tga) mimehandler(image/x-xbitmap)"
RDEPENDS:${PN} += "desktop-file-utils hicolor-icon-theme imlib2-loaders ld-linux-aarch64.so.1()(64bit) ld-linux-aarch64.so.1(GLIBC_2.17)(64bit) libImlib2.so.1()(64bit) libX11.so.6()(64bit) libXinerama.so.1()(64bit) libc.so.6(GLIBC_2.34)(64bit) libcurl.so.4()(64bit) libexif.so.12()(64bit) libjpeg-turbo libm.so.6()(64bit) libm.so.6(GLIBC_2.17)(64bit) libpng16.so.16()(64bit) libpng16.so.16(PNG16_0)(64bit)"
