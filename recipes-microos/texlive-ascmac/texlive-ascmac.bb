...
RPROVIDES:${PN} += "tex(ascgrp.tfm) tex(ascii10.tfm) tex(ascii36.tfm) tex(ascmac.map) tex(ascmac.sty) tex(tascmac.sty) texlive-ascmac"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(updmap.cfg) texlive texlive-ascmac-fonts texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
