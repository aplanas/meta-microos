...
RPROVIDES:${PN} += "tex(texnegar-common-kashida.tex) tex(texnegar-ini.tex) tex(texnegar-luatex.sty) tex(texnegar-xetex-kashida.tex) tex(texnegar-xetex.sty) tex(texnegar.sty) texlive-texnegar"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(array.sty) tex(environ.sty) tex(fontspec.sty) tex(graphicx.sty) tex(l3keys2e.sty) tex(newverbs.sty) tex(xcolor.sty) tex(xparse.sty) tex(zref-savepos.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
