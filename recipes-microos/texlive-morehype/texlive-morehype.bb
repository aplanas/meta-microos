...
RPROVIDES:${PN} += "tex(blog.sty) tex(blogdot.cfg) tex(blogdot.sty) tex(blogexec.sty) tex(blogligs.sty) tex(hypertoc.sty) tex(lnavicol.sty) tex(markblog.sty) tex(texlinks.sty) texlive-morehype"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(domore.sty) tex(dowith.sty) tex(fifinddo.sty) tex(langcode.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
