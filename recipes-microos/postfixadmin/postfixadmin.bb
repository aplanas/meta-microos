...
RPROVIDES:${PN} += "config(postfixadmin) postfixadmin"
RDEPENDS:${PN} += "/bin/bash /bin/sh /usr/bin/env /usr/bin/perl /usr/sbin/sendmail perl(DBI) perl(Email::Sender::Simple) perl(Email::Sender::Transport::SMTP) perl(Email::Simple) perl(Email::Simple::Creator) perl(Email::Valid) perl(Encode) perl(Fcntl) perl(File::Basename) perl(File::Path) perl(File::Temp) perl(Getopt::Long) perl(Getopt::Std) perl(IO) perl(IO::File) perl(LockFile::Simple) perl(Log::Log4perl) perl(MIME::EncWords) perl(POSIX) perl(Sys::Syslog) perl(Time::Local) perl(Try::Tiny) perl(strict) php php-mbstring php-phar php-spl php_any_db shadow"
