...
RPROVIDES:${PN} += "tex(tablor-xetex.sty) tex(tablor.cfg) tex(tablor.sty) texlive-tablor"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(babel.sty) tex(fancyvrb.sty) tex(filecontents.sty) tex(fontenc.sty) tex(graphicx.sty) tex(ifpdf.sty) tex(ifthen.sty) tex(ifxetex.sty) tex(pst-eps.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
