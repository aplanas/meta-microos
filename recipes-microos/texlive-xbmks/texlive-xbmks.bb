...
RPROVIDES:${PN} += "tex(xbmks-pdfmark.def) tex(xbmks-pdftex.def) tex(xbmks-xetex.def) tex(xbmks.sty) texlive-xbmks"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(hyperref.sty) tex(ifpdf.sty) tex(ifxetex.sty) tex(xkeyval.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
