...
RPROVIDES:${PN} += "tex(meetingmins.cls) texlive-meetingmins"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(article.cls) tex(enumitem.sty) tex(environ.sty) tex(fancyhdr.sty) tex(fontenc.sty) tex(geometry.sty) tex(lmodern.sty) tex(mathabx.sty) tex(xstring.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
