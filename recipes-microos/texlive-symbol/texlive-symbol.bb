...
RPROVIDES:${PN} += "tex(msyr.tfm) tex(psyr.tfm) tex(usy.map) tex(usyr.tfm) tex(uusy.fd) texlive-symbol"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(updmap.cfg) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin texlive-symbol-fonts"
