...
RPROVIDES:${PN} += "application() application(gammastep-indicator.desktop) gammastep-indicator gammastep-indicator(aarch-64) metainfo() metainfo(gammastep-indicator.appdata.xml)"
RDEPENDS:${PN} += "/bin/sh /usr/bin/python3 python(abi) python3dist(pygobject) python3dist(pyxdg) typelib(AyatanaAppIndicator3) typelib(GLib) typelib(GObject) typelib(Gtk)"
