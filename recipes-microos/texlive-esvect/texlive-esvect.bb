...
RPROVIDES:${PN} += "tex(esvect.map) tex(esvect.sty) tex(uesvect.fd) tex(vect10.tfm) tex(vect5.tfm) tex(vect6.tfm) tex(vect7.tfm) tex(vect8.tfm) tex(vect9.tfm) texlive-esvect"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(updmap.cfg) texlive texlive-esvect-fonts texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
