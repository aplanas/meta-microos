...
RPROVIDES:${PN} += "tex(gmRCS.sty) tex(gmampulex.sty) tex(gmbase.sty) tex(gmcommand.sty) tex(gmenvir.sty) tex(gmlogos.sty) tex(gmmeta.sty) tex(gmmw.sty) tex(gmnotonlypream.sty) tex(gmparts.sty) tex(gmrelsize.sty) tex(gmtypos.sty) tex(gmurl.sty) tex(gmutils.sty) texlive-gmutils"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(calc.sty) tex(expl3.sty) tex(graphicx.sty) tex(multicol.sty) tex(polski.sty) tex(xkeyval.sty) tex(xltxtra.sty) tex(xparse.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
