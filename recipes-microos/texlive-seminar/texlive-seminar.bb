...
RPROVIDES:${PN} += "tex(npsfont.sty) tex(sem-a4.sty) tex(sem-dem.sty) tex(sem-page.sty) tex(semcolor.sty) tex(semhelv.sty) tex(seminar.cls) tex(seminar.sty) tex(semlayer.sty) tex(semlcmss.sty) tex(semrot.sty) tex(slidesec.sty) tex(tvz-code.sty) tex(tvz-hax.sty) tex(tvz-user.sty) texlive-seminar"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(article.cls) tex(article.sty) tex(doc.sty) tex(fancybox.sty) tex(hyperref.sty) tex(pst-ovl.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
