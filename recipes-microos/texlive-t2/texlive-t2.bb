...
RPROVIDES:${PN} += "tex(alias-cmc.tex) tex(alias-wncy.tex) tex(citehack.sty) tex(cyralias.tex) tex(fnstcorr.tex) tex(mathtext.sty) tex(misccorr.sty) tex(t2a-mod1.enc) tex(t2a-mod2.enc) tex(t2a.enc) tex(t2b.enc) tex(t2c.enc) tex(x2.enc) texlive-t2"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(amssymb.sty) tex(enumerate.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
