...
RPROVIDES:${PN} += "tex(abbrevs.cfg) tex(abbrevs.sty) tex(achicago.sty) tex(attrib.sty) tex(blkcntrl.sty) tex(compsci.cfg) tex(compsci.sty) tex(dialogue.sty) tex(lips.sty) tex(moredefs.sty) tex(newclude.sty) tex(slemph.cfg) tex(slemph.sty) tex(titles.cfg) tex(titles.sty) texlive-frankenstein"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(relsize.sty) tex(verbatim.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
