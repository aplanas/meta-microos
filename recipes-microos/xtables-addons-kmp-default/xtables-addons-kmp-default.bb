...
RPROVIDES:${PN} += "kmod(compat_xtables.ko) kmod(xt_ACCOUNT.ko) kmod(xt_CHAOS.ko) kmod(xt_DELUDE.ko) kmod(xt_DHCPMAC.ko) kmod(xt_DNETMAP.ko) kmod(xt_ECHO.ko) kmod(xt_IPMARK.ko) kmod(xt_LOGMARK.ko) kmod(xt_PROTO.ko) kmod(xt_SYSRQ.ko) kmod(xt_TARPIT.ko) kmod(xt_asn.ko) kmod(xt_condition.ko) kmod(xt_fuzzy.ko) kmod(xt_geoip.ko) kmod(xt_iface.ko) kmod(xt_ipp2p.ko) kmod(xt_ipv4options.ko) kmod(xt_length2.ko) kmod(xt_lscan.ko) kmod(xt_pknock.ko) kmod(xt_psd.ko) kmod(xt_quota2.ko) multiversion(kernel) xtables-addons-kmp xtables-addons-kmp-default xtables-addons-kmp-default(aarch-64) xtables-addons-kmp-default-k6.3.4_1"
