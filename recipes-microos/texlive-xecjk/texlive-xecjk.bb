...
RPROVIDES:${PN} += "tex(full-stop.map) tex(fullwidth-stop.map) tex(han-simp.map) tex(han-trad.map) tex(xeCJK-listings.sty) tex(xeCJK.cfg) tex(xeCJK.sty) tex(xeCJKfntef.sty) tex(xunicode-addon.sty) tex(xunicode-extra.def) texlive-xecjk"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(ctexhook.sty) tex(expl3.sty) tex(fontspec.sty) tex(l3keys2e.sty) tex(listings.sty) tex(ulem.sty) tex(xparse.sty) tex(xtemplate.sty) tex(xunicode.sty) texlive texlive-ctex texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
