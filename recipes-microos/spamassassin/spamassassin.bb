...
RPROVIDES:${PN} += "spamassassin spamassassin(aarch-64)"
RDEPENDS:${PN} += "(curl >= 7.2.14 or wget >= 1.8.2) /bin/bash /bin/sh /usr/bin/mkdir /usr/bin/perl /usr/bin/touch fillup perl(Archive::Tar) perl(Error) perl(IO::Zlib) perl(LWP) perl(Mail::SpamAssassin) perl(Net::Server::PreForkSimple) re2c spamassassin-spamc"
