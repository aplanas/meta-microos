...
RPROVIDES:${PN} += "tex(mathsemantics-abbreviations.sty) tex(mathsemantics-commons.sty) tex(mathsemantics-manifolds.sty) tex(mathsemantics-names.sty) tex(mathsemantics-optimization.sty) tex(mathsemantics-semantic.sty) tex(mathsemantics-syntax.sty) tex(mathsemantics.sty) texlive-mathsemantics"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(amssymb.sty) tex(calc.sty) tex(ifthen.sty) tex(ifxetex.sty) tex(mathtools.sty) tex(xifthen.sty) tex(xparse.sty) tex(xspace.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
