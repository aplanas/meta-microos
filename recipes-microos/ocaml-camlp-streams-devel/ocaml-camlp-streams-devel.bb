...
RPROVIDES:${PN} += "ocaml(Genlex) ocaml(Stream) ocaml-camlp-streams-devel ocaml-camlp-streams-devel(aarch-64) ocamlfind(camlp-streams) ocamlx(Genlex) ocamlx(Stream)"
RDEPENDS:${PN} += "ocaml(CamlinternalFormatBasics) ocaml(Stdlib) ocaml(Stdlib__Genlex) ocaml(Stdlib__Stream) ocaml-camlp-streams ocamlx(Stdlib__Genlex) ocamlx(Stdlib__Stream)"
