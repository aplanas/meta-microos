...
RPROVIDES:${PN} += "tex(pgf-PeriodicTable.sty) tex(pgfPT.backcolors.keys.tex) tex(pgfPT.buildcell.tex) tex(pgfPT.coordinates.tex) tex(pgfPT.data.tex) tex(pgfPT.drawing.keys.tex) tex(pgfPT.formatNumbers.tex) tex(pgfPT.input.library.tex) tex(pgfPT.labels.tex) tex(pgfPT.names.tex) tex(pgfPT.process.language.tex) texlive-pgf-periodictable"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(fontenc.sty) tex(tikz.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
