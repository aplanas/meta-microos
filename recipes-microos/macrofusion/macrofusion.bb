...
RPROVIDES:${PN} += "application() application(macrofusion.desktop) macrofusion mimehandler(application/x-mfusion) mimehandler(image/jpeg) mimehandler(image/tiff)"
RDEPENDS:${PN} += "/usr/bin/python3 enblend-enfuse hugin python3-Pillow python3-cairo python3-gobject-Gdk typelib(GExiv2) typelib(GObject) typelib(Gdk) typelib(GdkPixbuf) typelib(Gtk)"
