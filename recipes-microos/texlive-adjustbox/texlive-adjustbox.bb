...
RPROVIDES:${PN} += "tex(adjcalc.sty) tex(adjustbox.sty) tex(tc-dvips.def) tex(tc-pdftex.def) tex(tc-pgf.def) tex(tc-xetex.def) tex(trimclip.sty) texlive-adjustbox"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(calc.sty) tex(collectbox.sty) tex(graphicx.sty) tex(ifoddpage.sty) tex(pgf.sty) tex(pgfmath.sty) tex(varwidth.sty) tex(xkeyval.sty) texlive texlive-collectbox texlive-filesystem texlive-graphics texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin texlive-xkeyval"
