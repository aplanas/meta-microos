...
RPROVIDES:${PN} += "tex(pkgloader-cls-pkg.sty) tex(pkgloader-dry.sty) tex(pkgloader-early.sty) tex(pkgloader-error.sty) tex(pkgloader-false.sty) tex(pkgloader-late.sty) tex(pkgloader-recommended.sty) tex(pkgloader-true.sty) tex(pkgloader.sty) texlive-pkgloader"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(etoolbox.sty) tex(expl3.sty) tex(filecontents.sty) tex(l3keys2e.sty) tex(lt3graph.sty) tex(withargs.sty) tex(xparse.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
