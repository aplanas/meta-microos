...
RPROVIDES:${PN} += "tex(muling.cls) texlive-muling"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(biblatex.sty) tex(book.cls) tex(expex.sty) tex(fancyhdr.sty) tex(forest.sty) tex(hyperref.sty) tex(leipzig.sty) tex(ling-macros.sty) tex(tipa.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
