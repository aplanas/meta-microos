...
RPROVIDES:${PN} += "config(mylvmbackup) mylvmbackup perl(backupfailure) perl(logerr) perl(preflush)"
RDEPENDS:${PN} += "/usr/bin/perl perl(Config::IniFiles) perl(DBD::mysql) perl(DBI) perl(Date::Format) perl(Fcntl) perl(File::Basename) perl(File::Copy) perl(File::Copy::Recursive) perl(File::Path) perl(File::Temp) perl(Getopt::Long) perl(Sys::Hostname)"
