...
RPROVIDES:${PN} += "tex(EBGaramond-Italic--oml-ebgaramond.tfm) tex(EBGaramond-Maths.map) tex(OMLEBGaramond-Maths.fd) tex(a_w5zv5t.enc) tex(ebgaramond-maths.sty) texlive-ebgaramond-maths"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(ebgaramond.sty) tex(updmap.cfg) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
