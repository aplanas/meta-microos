...
RPROVIDES:${PN} += "tex(amigapl.def) tex(mazovia.def) tex(omlplcm.fd) tex(omlplm.fd) tex(omsplsy.fd) tex(omxplex.fd) tex(ot1patch.sty) tex(ot4ccr.fd) tex(ot4cmdh.fd) tex(ot4cmfib.fd) tex(ot4cmfr.fd) tex(ot4cmr.fd) tex(ot4cmss.fd) tex(ot4cmtt.fd) tex(plprefix.sty) tex(polski.sty) tex(qxenc.def) texlive-polski"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(ot4enc.def) texlive texlive-filesystem texlive-hyphen-polish texlive-kpathsea texlive-kpathsea-bin texlive-pl texlive-scripts texlive-scripts-bin"
