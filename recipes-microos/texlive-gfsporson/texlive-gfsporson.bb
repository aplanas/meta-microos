...
RPROVIDES:${PN} += "tex(gfsporson.map) tex(gfsporson.sty) tex(gporsonrg6a.tfm) tex(gporsonrg6a.vf) tex(gporsonrg6r.tfm) tex(lgrporson.fd) tex(porsonel.enc) texlive-gfsporson"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(updmap.cfg) texlive texlive-filesystem texlive-gfsporson-fonts texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
