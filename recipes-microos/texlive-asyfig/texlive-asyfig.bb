...
RPROVIDES:${PN} += "tex(asyalign.sty) tex(asyfig.sty) tex(asyprocess.sty) texlive-asyfig"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(catchfile.sty) tex(color.sty) tex(graphicx.sty) tex(ifmtarg.sty) tex(ifpdf.sty) tex(ifplatform.sty) tex(preview.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
