...
RPROVIDES:${PN} += "cmake(CapnProto) libcapnp-devel libcapnp-devel(aarch-64) pkgconfig(capnp) pkgconfig(capnp-json) pkgconfig(capnp-rpc) pkgconfig(capnp-websocket) pkgconfig(capnpc) pkgconfig(kj) pkgconfig(kj-async) pkgconfig(kj-gzip) pkgconfig(kj-http) pkgconfig(kj-test) pkgconfig(kj-tls)"
RDEPENDS:${PN} += "/usr/bin/pkg-config libcapnp-0_10_4"
