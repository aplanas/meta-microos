...
RPROVIDES:${PN} += "tex(accenti.sty) tex(dblfont.sty) tex(envmath.sty) tex(evenpage.sty) tex(graphfig.sty) tex(mathcmd.sty) tex(quotes.sty) tex(sobolev.sty) texlive-bosisio"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(float.sty) tex(graphics.sty) tex(subfigure.sty) tex(xspace.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
