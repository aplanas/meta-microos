...
RPROVIDES:${PN} += "application() application(org.gnome.Meld.desktop) meld metainfo() metainfo(org.gnome.Meld.appdata.xml) mimehandler(application/x-meld-comparison)"
RDEPENDS:${PN} += "/usr/bin/python3 patch python(abi) python3-cairo python3-gobject-Gdk typelib(GLib) typelib(GObject) typelib(Gdk) typelib(GdkPixbuf) typelib(Gio) typelib(Gtk) typelib(GtkSource) typelib(Pango)"
