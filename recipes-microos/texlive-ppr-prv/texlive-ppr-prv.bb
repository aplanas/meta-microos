...
RPROVIDES:${PN} += "tex(HAP-ppr-prv.def) tex(ppr-prv.cls) texlive-ppr-prv"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(article.cls) tex(float.sty) tex(geometry.sty) tex(graphicx.sty) tex(hyperref.sty) tex(ifpdf.sty) tex(keyval.sty) tex(pstricks.sty) tex(textcomp.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
