...
RPROVIDES:${PN} += "tex(fixmetodonotes.sty) texlive-fixmetodonotes"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(color.sty) tex(fix-cm.sty) tex(graphicx.sty) tex(tocloft.sty) tex(transparent.sty) tex(ulem.sty) tex(watermark.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
