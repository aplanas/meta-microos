...
RPROVIDES:${PN} += "tex(assy.tfm) tex(assyr.sty) tex(assyrb10.tfm) tex(serto.sty) tex(serto10.tfm) tex(sertob10.tfm) tex(syriac.map) tex(syriac.sty) tex(uassyr.fd) tex(userto.fd) texlive-aramaic-serto"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(updmap.cfg) texlive texlive-aramaic-serto-fonts texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
