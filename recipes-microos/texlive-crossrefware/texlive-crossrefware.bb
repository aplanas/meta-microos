...
RPROVIDES:${PN} += "tex(bibdoiadd.cfg) tex(ltx2crossrefxml.cfg) texlive-crossrefware"
RDEPENDS:${PN} += "/bin/sh /usr/bin/perl coreutils ed findutils grep perl(BibTeX::Parser) perl(BibTeX::Parser::Author) perl(Cwd) perl(File::Basename) perl(File::Spec) perl(FileHandle) perl(Getopt::Long) perl(Getopt::Std) perl(IO::File) perl(LWP::Simple) perl(LWP::UserAgent) perl(LaTeX::ToUnicode) perl(POSIX) perl(URI::Escape) perl(strict) perl(utf8) perl(warnings) sed texlive texlive-crossrefware-bin texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
