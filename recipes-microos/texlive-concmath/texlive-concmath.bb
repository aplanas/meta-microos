...
RPROVIDES:${PN} += "tex(concmath.sty) tex(omlccm.fd) tex(omlccr.fd) tex(omsccr.fd) tex(omsccsy.fd) tex(omxccex.fd) tex(ot1ccr.fd) tex(ucca.fd) tex(uccb.fd) texlive-concmath"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(amsfonts.sty) tex(amssymb.sty) tex(exscale.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
