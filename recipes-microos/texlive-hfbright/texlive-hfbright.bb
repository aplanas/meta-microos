...
RPROVIDES:${PN} += "tex(hfbright.map) tex(hfmital.enc) tex(hfmsa.enc) tex(hfmsb.enc) tex(hfmsym.enc) tex(hfot1.enc) texlive-hfbright"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(updmap.cfg) texlive texlive-filesystem texlive-hfbright-fonts texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
