...
RPROVIDES:${PN} += "application() application(com.github.maoschanz.drawing.desktop) drawing metainfo() metainfo(com.github.maoschanz.drawing.appdata.xml) mimehandler(image/bmp) mimehandler(image/jpeg) mimehandler(image/png)"
RDEPENDS:${PN} += "/usr/bin/python3 python3 python3-gobject python3-gobject-Gdk python3-gobject-cairo python3-pycairo typelib(GLib) typelib(Gdk) typelib(GdkPixbuf) typelib(Gio) typelib(Gtk) typelib(Pango) typelib(PangoCairo)"
