...
RPROVIDES:${PN} += "tex(letterspace.sty) tex(microtype-luatex.def) tex(microtype-pdftex.def) tex(microtype-show.sty) tex(microtype-xetex.def) tex(microtype.cfg) tex(microtype.sty) tex(mt-CharisSIL.cfg) tex(mt-EBGaramond.cfg) tex(mt-LatinModernRoman.cfg) tex(mt-NewComputerModern.cfg) tex(mt-Palatino.cfg) tex(mt-TU-basic.cfg) tex(mt-TU-empty.cfg) tex(mt-bch.cfg) tex(mt-blg.cfg) tex(mt-cmr.cfg) tex(mt-euf.cfg) tex(mt-eur.cfg) tex(mt-eus.cfg) tex(mt-msa.cfg) tex(mt-msb.cfg) tex(mt-mvs.cfg) tex(mt-pmn.cfg) tex(mt-ppl.cfg) tex(mt-ptm.cfg) tex(mt-ugm.cfg) tex(mt-zpeu.cfg) texlive-microtype"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(etoolbox.sty) tex(graphicx.sty) tex(iftex.sty) tex(keyval.sty) tex(luaotfload.sty) tex(luatexbase.sty) tex(xcolor.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
