...
RPROVIDES:${PN} += "config(texlive-latexmk) texlive-latexmk"
RDEPENDS:${PN} += "/bin/sh /usr/bin/perl coreutils ed findutils grep perl(Config) perl(Cwd) perl(Digest::MD5) perl(Encode) perl(File::Basename) perl(File::Copy) perl(File::Find) perl(File::Glob) perl(File::Path) perl(File::Spec::Functions) perl(FileHandle) perl(List::Util) perl(Time::HiRes) perl(Unicode::Normalize) perl(feature) perl(sigtrap) perl(utf8) perl(vars) perl(warnings) sed texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-latexmk-bin texlive-scripts texlive-scripts-bin"
