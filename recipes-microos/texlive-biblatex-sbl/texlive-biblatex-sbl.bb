...
RPROVIDES:${PN} += "tex(american-sbl.lbx) tex(biblatex-sbl.def) tex(english-sbl.lbx) tex(german-sbl.lbx) tex(sbl-paper.sty) tex(sbl.bbx) tex(sbl.cbx) tex(spanish-sbl.lbx) texlive-biblatex-sbl"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(american.lbx) tex(biblatex.sty) tex(bibleref-parse.sty) tex(fancyhdr.sty) tex(footmisc.sty) tex(geometry.sty) tex(hyperref.sty) tex(imakeidx.sty) tex(setspace.sty) tex(standard.bbx) tex(textcase.sty) tex(titlesec.sty) tex(titletoc.sty) tex(xparse.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
