...
RPROVIDES:${PN} += "tex(marathi.sty) tex(namuna-article.tex) tex(namuna-beamer.tex) tex(namuna-book.tex) tex(namuna-letter.tex) tex(namuna-para.tex) tex(namuna-report.tex) texlive-marathi"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(babel.sty) tex(csquotes.sty) tex(fontspec.sty) tex(iftex.sty) tex(pgfkeys.sty) tex(setspace.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
