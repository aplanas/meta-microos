...
RPROVIDES:${PN} += "tex(atfi-dvipdfmx.def) tex(atfi-dvips.def) tex(atfi-luatex.def) tex(atfi-pdftex.def) tex(attachfile2.sty) texlive-attachfile2"
RDEPENDS:${PN} += "/bin/sh /usr/bin/perl coreutils ed findutils grep perl(Digest::MD5) perl(Getopt::Long) perl(POSIX) perl(strict) sed tex(color.sty) tex(hycolor.sty) tex(hyperref.sty) tex(iftex.sty) tex(infwarerr.sty) tex(keyval.sty) tex(kvoptions.sty) tex(ltxcmds.sty) tex(pdfescape.sty) tex(pdftexcmds.sty) texlive texlive-attachfile2-bin texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
