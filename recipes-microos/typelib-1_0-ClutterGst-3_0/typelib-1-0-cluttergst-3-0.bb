...
RPROVIDES:${PN} += "typelib(ClutterGst) typelib-1_0-ClutterGst-3_0 typelib-1_0-ClutterGst-3_0(aarch-64)"
RDEPENDS:${PN} += "libclutter-gst-3.0.so.0()(64bit) typelib(Atk) typelib(Clutter) typelib(Cogl) typelib(CoglPango) typelib(GL) typelib(GLib) typelib(GModule) typelib(GObject) typelib(GdkPixbuf) typelib(Gio) typelib(Gst) typelib(GstAudio) typelib(GstBase) typelib(GstPbutils) typelib(GstVideo) typelib(HarfBuzz) typelib(Json) typelib(Pango) typelib(PangoCairo) typelib(cairo) typelib(freetype2)"
