...
RPROVIDES:${PN} += "tex(ecvbg.def) tex(ecvca.def) tex(ecvcs.def) tex(ecvda.def) tex(ecvde.def) tex(ecven.def) tex(ecves.def) tex(ecvet.def) tex(ecvfi.def) tex(ecvfr.def) tex(ecvgl.def) tex(ecvgr.def) tex(ecvhu.def) tex(ecvis.def) tex(ecvit.def) tex(ecvlt.def) tex(ecvlv.def) tex(ecvmt.def) tex(ecvnl.def) tex(ecvno.def) tex(ecvpl.def) tex(ecvpt.def) tex(ecvro.def) tex(ecvsk.def) tex(ecvsl.def) tex(ecvsr.def) tex(ecvsv.def) tex(europecv.cls) texlive-europecv"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(array.sty) tex(article.cls) tex(booktabs.sty) tex(combelow.sty) tex(etoolbox.sty) tex(fancyhdr.sty) tex(hyperref.sty) tex(longtable.sty) tex(totpages.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
