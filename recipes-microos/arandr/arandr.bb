...
RPROVIDES:${PN} += "application() application(arandr.desktop) arandr python3.10dist(arandr) python3dist(arandr)"
RDEPENDS:${PN} += "/usr/bin/python3 python(abi) python3-gobject python3-gobject-Gdk python3-pycairo typelib(GLib) typelib(GObject) typelib(Gdk) typelib(Gtk) typelib(Pango) typelib(PangoCairo) xrandr"
