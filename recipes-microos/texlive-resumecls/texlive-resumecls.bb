...
RPROVIDES:${PN} += "tex(resumecls.cls) texlive-resumecls"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(color.sty) tex(ctexart.cls) tex(fancyhdr.sty) tex(geometry.sty) tex(hyperref.sty) tex(natbib.sty) tex(tabularx.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
