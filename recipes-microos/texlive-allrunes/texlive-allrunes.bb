...
RPROVIDES:${PN} += "tex(allrunes.map) tex(allrunes.sty) tex(ara.fd) tex(arc.fd) tex(arl.fd) tex(arm.fd) tex(arn.fd) tex(art.fd) texlive-allrunes"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(ifthen.sty) tex(updmap.cfg) texlive texlive-allrunes-fonts texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
