...
RPROVIDES:${PN} += "tex(pndraw.sty) tex(pndraw.tex) tex(pnets.sty) tex(pnets.tex) tex(pntext.sty) tex(pntext.tex) tex(pnversion.tex) texlive-petri-nets"
RDEPENDS:${PN} += "/bin/sh /usr/bin/perl coreutils ed findutils grep perl(Digest::MD5) perl(Getopt::Long) perl(strict) sed tex(amsfonts.sty) tex(graphicx.sty) tex(ifpdf.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-petri-nets-bin texlive-scripts texlive-scripts-bin"
