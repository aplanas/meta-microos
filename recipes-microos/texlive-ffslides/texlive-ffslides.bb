...
RPROVIDES:${PN} += "tex(ffslides.cls) texlive-ffslides"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(article.cls) tex(calc.sty) tex(geometry.sty) tex(graphics.sty) tex(hyperref.sty) tex(ifthen.sty) tex(lastpage.sty) tex(pstricks.sty) tex(twoopt.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
