...
RPROVIDES:${PN} += "tex(mptopdf.tex) texlive-mptopdf"
RDEPENDS:${PN} += "/bin/sh /usr/bin/perl coreutils ed findutils grep perl(Config) perl(File::Basename) perl(Getopt::Long) perl(strict) sed tex(language.dat) tex(language.dat.lua) tex(language.def) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-mptopdf-bin texlive-plain texlive-scripts texlive-scripts-bin"
