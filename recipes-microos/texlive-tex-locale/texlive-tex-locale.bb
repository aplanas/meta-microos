...
RPROVIDES:${PN} += "tex(tex-locale-encodings.def) tex(tex-locale-scripts-enc.def) tex(tex-locale-support.def) tex(tex-locale.sty) tex(tex-locale.tex) texlive-tex-locale"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(CJK.sty) tex(CJKutf8.sty) tex(babel.sty) tex(datetime2.sty) tex(etoolbox.sty) tex(fontenc.sty) tex(fontspec.sty) tex(ifluatex.sty) tex(ifxetex.sty) tex(inputenc.sty) tex(pinyin.sty) tex(polyglossia.sty) tex(texosquery.sty) tex(tracklang-scripts.sty) tex(tracklang.sty) tex(xeCJK.sty) tex(xfor.sty) tex(xkeyval.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
