...
RPROVIDES:${PN} += "tex(strukdoc.sty) tex(struktex.sty) tex(struktxf.sty) tex(struktxp.sty) texlive-struktex"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(color.sty) tex(curves.sty) tex(hyperref.sty) tex(ifpdf.sty) tex(ifthen.sty) tex(nameref.sty) tex(pict2e.sty) tex(url.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
