...
RPROVIDES:${PN} += "WALinuxAgent config(python-azure-agent) python-azure-agent python3.10dist(walinuxagent) python3dist(walinuxagent)"
RDEPENDS:${PN} += "/bin/sh /usr/bin/env /usr/bin/python3 eject grep iptables logrotate openssh openssl pwdutils python(abi) python3-distro python3-pyasn1 python3-xml sudo systemd sysvinit-tools util-linux wicked"
