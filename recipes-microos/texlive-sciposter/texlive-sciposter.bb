...
RPROVIDES:${PN} += "tex(paperb0.cfg) tex(paperb1.cfg) tex(paperb2.cfg) tex(paperb3.cfg) tex(papercustom.cfg) tex(paperra0.cfg) tex(paperra1.cfg) tex(paperra2.cfg) tex(sciposter.cls) texlive-sciposter"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(a0size.sty) tex(article.cls) tex(boxedminipage.sty) tex(color.sty) tex(graphics.sty) tex(ifthen.sty) tex(lettrine.sty) tex(shadow.sty) tex(times.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
