...
RPROVIDES:${PN} += "tex(diadia.cfg) tex(diadia.sty) texlive-diadia"
RDEPENDS:${PN} += "/bin/sh /usr/bin/texlua coreutils ed findutils grep sed tex(amsmath.sty) tex(amssymb.sty) tex(booktabs.sty) tex(calc.sty) tex(colortbl.sty) tex(environ.sty) tex(ifthen.sty) tex(multicol.sty) tex(pgfcalendar.sty) tex(pgfplots.sty) tex(pgfplotstable.sty) tex(tabularx.sty) tex(tcolorbox.sty) tex(translations.sty) tex(xkeyval.sty) texlive texlive-diadia-bin texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
