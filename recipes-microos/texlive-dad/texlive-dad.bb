...
RPROVIDES:${PN} += "tex(OT1dad.fd) tex(T1dad.fd) tex(dad.map) tex(dad.sty) tex(dadreal.tfm) tex(dadrealbold.tfm) tex(dadrealmono.tfm) texlive-dad"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(luatex85.sty) tex(updmap.cfg) texlive texlive-dad-fonts texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
