...
RPROVIDES:${PN} += "tex(p2e-dvipdfm.def) tex(p2e-dvipdfmx.def) tex(p2e-dvips.def) tex(p2e-luatex.def) tex(p2e-pctex32.def) tex(p2e-pctexps.def) tex(p2e-pdftex.def) tex(p2e-textures.def) tex(p2e-vtex.def) tex(p2e-xetex.def) tex(pict2e.cfg) tex(pict2e.sty) texlive-pict2e"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(trig.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
