...
RPROVIDES:${PN} += "tex(color-ltx.sty) tex(colorspace-patches-tmp-ltx.sty) tex(hgeneric-testphase.def) tex(hyperref-colorschemes.def) tex(l3backend-testphase-dvipdfmx.def) tex(l3backend-testphase-dvips.def) tex(l3backend-testphase-dvisvgm.def) tex(l3backend-testphase-luatex.def) tex(l3backend-testphase-pdftex.def) tex(l3backend-testphase-xetex.def) tex(l3pdffield-testphase.sty) tex(l3ref-tmp.sty) tex(output-patches-tmp-ltx.sty) tex(pdfmanagement-firstaid.sty) tex(pdfmanagement-testphase.sty) tex(xcolor-patches-tmp-ltx.sty) texlive-pdfmanagement-testphase"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(bookmark.sty) tex(etoolbox.sty) tex(l3draw.sty) tex(tagpdf-base.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
