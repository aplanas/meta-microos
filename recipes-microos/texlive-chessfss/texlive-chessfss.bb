...
RPROVIDES:${PN} += "tex(chess-board-example-enc.enc) tex(chess-fig-example-enc.enc) tex(chessfss.sty) tex(lsb1enc.def) tex(lsb1skak.fd) tex(lsb1skaknew.fd) tex(lsb2enc.def) tex(lsb2skak.fd) tex(lsb2skaknew.fd) tex(lsb3enc.def) tex(lsb3skak.fd) tex(lsb3skaknew.fd) tex(lsbc1enc.def) tex(lsbc1skaknew.fd) tex(lsbc2enc.def) tex(lsbc2skaknew.fd) tex(lsbc3enc.def) tex(lsbc3skaknew.fd) tex(lsbc4enc.def) tex(lsbc4skaknew.fd) tex(lsbc5enc.def) tex(lsbc5skaknew.fd) tex(lsbenc.def) tex(lsbskak.fd) tex(lsbskaknew.fd) tex(lsfenc.def) tex(lsfskak.fd) tex(lsfskaknew.fd) tex(lsienc.def) tex(lsiskak.fd) tex(lsiskaknew.fd) texlive-chessfss"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(ifthen.sty) tex(xkeyval.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
