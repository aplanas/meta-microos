...
RPROVIDES:${PN} += "tex(semproc.cls) texlive-semproc"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(biblatex.sty) tex(bookmark.sty) tex(csquotes.sty) tex(etoc.sty) tex(etoolbox.sty) tex(scrreprt.cls) tex(xcolor.sty) tex(xkeyval.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
