...
RPROVIDES:${PN} += "tex(libertinesups.tfm) tex(superiors.map) tex(superiors.sty) tex(sups.enc) texlive-superiors"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(fontenc.sty) tex(pgffor.sty) tex(updmap.cfg) tex(xkeyval.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin texlive-superiors-fonts"
