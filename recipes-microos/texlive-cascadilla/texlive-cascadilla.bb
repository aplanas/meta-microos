...
RPROVIDES:${PN} += "tex(cascadilla.cls) texlive-cascadilla"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(article.cls) tex(caption.sty) tex(fancyhdr.sty) tex(geometry.sty) tex(ifthen.sty) tex(indentfirst.sty) tex(natbib.sty) tex(times.sty) tex(titlesec.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
