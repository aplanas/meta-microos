...
RPROVIDES:${PN} += "tex(amsbst.sty) tex(amsrefs.sty) tex(ifoption.sty) tex(mathscinet.sty) tex(pcatcode.sty) tex(rkeyval.sty) tex(textcmds.sty) texlive-amsrefs"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(backref.sty) tex(hyperref.sty) tex(url.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
