...
RPROVIDES:${PN} += "tex(pdfslide.cfg) tex(pdfslide.sty) tex(slide.clo) texlive-pdfslide"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(amsbsy.sty) tex(amssymb.sty) tex(colortbl.sty) tex(fancybox.sty) tex(graphicx.sty) tex(ifthen.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
