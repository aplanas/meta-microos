...
RPROVIDES:${PN} += "application() application(gnome-dvb-control.desktop) application(gnome-dvb-setup.desktop) python-gnome-dvb-daemon python-gnome-dvb-daemon(aarch-64) python-gnomedvb"
RDEPENDS:${PN} += "/bin/sh /usr/bin/env gnome-dvb-daemon python(abi) typelib(GLib) typelib(GObject) typelib(Gdk) typelib(GdkPixbuf) typelib(Gio) typelib(Gtk) typelib(Pango)"
