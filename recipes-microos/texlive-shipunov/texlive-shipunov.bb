...
RPROVIDES:${PN} += "tex(add2-shipunov.sty) tex(altverse.sty) tex(autolist.sty) tex(biokey.sty) tex(biolist.sty) tex(boldline.sty) tex(cassete.cls) tex(classif2.sty) tex(dline-shipunov.sty) tex(drcaps.sty) tex(etiketka.cls) tex(flower.sty) tex(isyntax.sty) tex(numerus.sty) tex(punct.sty) tex(qqru-shipunov.sty) tex(sltables.sty) tex(starfn.sty) tex(textfrac-shipunov.sty) texlive-shipunov"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(array.sty) tex(article.cls) tex(calc.sty) tex(footnpag.sty) tex(ifthen.sty) tex(lineno.sty) tex(syntonly.sty) tex(xspace.sty) tex(xtab.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
