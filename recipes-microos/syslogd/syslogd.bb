...
RPROVIDES:${PN} += "config(syslogd) sysklogd syslog syslogd syslogd(aarch-64) sysvinit(syslog)"
RDEPENDS:${PN} += "/bin/bash /bin/sh /usr/bin/mkdir /usr/bin/touch fillup group(news) klogd ld-linux-aarch64.so.1()(64bit) ld-linux-aarch64.so.1(GLIBC_2.17)(64bit) libc.so.6(GLIBC_2.34)(64bit) libsystemd.so.0()(64bit) libsystemd.so.0(LIBSYSTEMD_209)(64bit) permissions syslog-service systemd user(news)"
