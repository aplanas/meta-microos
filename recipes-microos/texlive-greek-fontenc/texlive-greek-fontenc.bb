...
RPROVIDES:${PN} += "tex(alphabeta-lgr.def) tex(alphabeta-tuenc.def) tex(alphabeta.sty) tex(greek-euenc.def) tex(greek-fontenc.def) tex(lgrenc.def) tex(puenc-greek.def) tex(textalpha.sty) tex(tuenc-greek.def) texlive-greek-fontenc"
RDEPENDS:${PN} += "/bin/sh /usr/bin/lua coreutils ed findutils grep sed texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
