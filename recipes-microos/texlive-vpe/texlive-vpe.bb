...
RPROVIDES:${PN} += "tex(vpe.sty) texlive-vpe"
RDEPENDS:${PN} += "/bin/sh /usr/bin/perl coreutils ed findutils grep perl(Cwd) perl(Getopt::Long) perl(strict) sed tex(color.sty) tex(keyval.sty) tex(pifont.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin texlive-vpe-bin"
