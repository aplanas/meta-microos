...
RPROVIDES:${PN} += "tex(afterpackage.sty) tex(dcounter.sty) tex(desclist.sty) tex(extdash.sty) tex(manyfoot.sty) tex(mboxfill.sty) tex(nccbbb.sty) tex(nccboxes.sty) tex(ncccomma.sty) tex(ncccropbox.sty) tex(ncccropmark.sty) tex(nccfancyhdr.sty) tex(nccfloats.sty) tex(nccfoots.sty) tex(nccmath.sty) tex(nccparskip.sty) tex(nccpic.sty) tex(nccrules.sty) tex(nccsect.sty) tex(nccstretch.sty) tex(nccthm.sty) tex(textarea.sty) tex(tocenter.sty) tex(topsection.sty) tex(watermark.sty) texlive-ncctools"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(amsgen.sty) tex(amsmath.sty) tex(graphicx.sty) tex(perpage.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
