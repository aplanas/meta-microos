...
RPROVIDES:${PN} += "tex(ieee-alphabetic.bbx) tex(ieee-alphabetic.cbx) tex(ieee.bbx) tex(ieee.cbx) tex(magyar-ieee.lbx) texlive-biblatex-ieee"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(alphabetic.cbx) tex(numeric-comp.bbx) tex(numeric-comp.cbx) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
