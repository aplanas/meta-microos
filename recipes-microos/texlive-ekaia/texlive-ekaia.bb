...
RPROVIDES:${PN} += "tex(ekaia.sty) texlive-ekaia"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(babel.sty) tex(basque-date.sty) tex(ccicons.sty) tex(fancyhdr.sty) tex(geometry.sty) tex(indentfirst.sty) tex(sectsty.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
