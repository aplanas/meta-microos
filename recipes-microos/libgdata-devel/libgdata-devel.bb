...
RPROVIDES:${PN} += "libgdata-devel libgdata-devel(aarch-64) pkgconfig(libgdata)"
RDEPENDS:${PN} += "/usr/bin/pkg-config libgdata22 pkgconfig(gcr-4) pkgconfig(gio-2.0) pkgconfig(gio-unix-2.0) pkgconfig(glib-2.0) pkgconfig(goa-1.0) pkgconfig(gobject-2.0) pkgconfig(gthread-2.0) pkgconfig(json-glib-1.0) pkgconfig(libsoup-2.4) pkgconfig(libxml-2.0) typelib-1_0-GData-0_0"
