...
RPROVIDES:${PN} += "tex(pxbabel.sty) tex(pxbase.def) tex(pxbase.sty) tex(pxbasenc.def) tex(pxbsjc.def) tex(pxbsjc1.def) tex(pxjsfenc.def) tex(upkcat.sty) texlive-pxbase"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(babel.sty) tex(bxbase.sty) tex(ifptex.sty) tex(ifuptex.sty) tex(rlbabel.def) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
