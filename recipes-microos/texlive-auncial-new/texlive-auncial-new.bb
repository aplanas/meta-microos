...
RPROVIDES:${PN} += "tex(allauncl.sty) tex(auncial.map) tex(auncial.sty) tex(auncl10.tfm) tex(aunclb10.tfm) tex(b1auncl.fd) texlive-auncial-new"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(updmap.cfg) texlive texlive-auncial-new-fonts texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
