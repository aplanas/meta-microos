...
RPROVIDES:${PN} += "tex(scanpages.map) tex(scanpages.sty) tex(scanwipe.tfm) tex(uscanwipe.fd) texlive-scanpages"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(etoolbox.sty) tex(fp-basic.sty) tex(graphicx.sty) tex(ifpdf.sty) tex(pgffor.sty) tex(updmap.cfg) tex(xcolor.sty) tex(xkeyval.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scanpages-fonts texlive-scripts texlive-scripts-bin"
