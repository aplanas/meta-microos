...
RPROVIDES:${PN} += "tex(DSSerif-Bold.tfm) tex(DSSerif.map) tex(DSSerif.tfm) tex(DSSerifUni-Bold.tfm) tex(DSSerifUni.tfm) tex(dsserif.sty) tex(udsserif.fd) texlive-dsserif"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(updmap.cfg) tex(xkeyval.sty) texlive texlive-dsserif-fonts texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
