...
RPROVIDES:${PN} += "perl(LWP::ConsoleLogger) perl(LWP::ConsoleLogger::Easy) perl(LWP::ConsoleLogger::Everywhere) perl-LWP-ConsoleLogger"
RDEPENDS:${PN} += "perl(:MODULE_COMPAT_5.36.0) perl(Class::Method::Modifiers) perl(Data::Printer) perl(DateTime) perl(HTML::Restrict) perl(HTTP::Body) perl(HTTP::CookieMonster) perl(HTTP::Request) perl(HTTP::Response) perl(JSON::MaybeXS) perl(List::AllUtils) perl(Log::Dispatch) perl(Module::Load::Conditional) perl(Module::Runtime) perl(Moo) perl(MooX::StrictConstructor) perl(Parse::MIME) perl(Ref::Util) perl(String::Trim) perl(Sub::Exporter) perl(Term::Size::Any) perl(Text::SimpleTable::AutoWidth) perl(Try::Tiny) perl(Types::Common::Numeric) perl(Types::Standard) perl(URI::QueryParam) perl(XML::Simple)"
