...
RPROVIDES:${PN} += "tex(brh-kan.map) tex(itrans-dvn.map) tex(itrans-iast.map) tex(itrans-kan.map) tex(itrans-sankan.map) tex(itrans-santel.map) tex(itrans-sdvn.map) tex(itrans-tamil.map) tex(itrans-tel.map) texlive-xetex-itrans"
