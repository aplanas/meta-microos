...
RPROVIDES:${PN} += "tex(tagpdf-base.sty) tex(tagpdf-debug-generic.sty) tex(tagpdf-debug-lua.sty) tex(tagpdf-debug.sty) tex(tagpdf-luatex.def) tex(tagpdf-mc-code-generic.sty) tex(tagpdf-mc-code-lua.sty) tex(tagpdf-ns-latex-book.def) tex(tagpdf-ns-latex-inline.def) tex(tagpdf-ns-latex.def) tex(tagpdf-ns-mathml.def) tex(tagpdf-ns-pdf.def) tex(tagpdf-ns-pdf2.def) tex(tagpdf.sty) tex(tagpdfdocu-patches.sty) texlive-tagpdf"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(etoolbox.sty) tex(l3ref-tmp.sty) tex(output-patches-tmp-ltx.sty) tex(pdfmanagement-testphase.sty) tex(xpatch.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
