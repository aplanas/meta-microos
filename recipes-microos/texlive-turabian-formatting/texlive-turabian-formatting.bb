...
RPROVIDES:${PN} += "tex(turabian-formatting.sty) tex(turabian-researchpaper.cls) tex(turabian-thesis.cls) texlive-turabian-formatting"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(article.cls) tex(book.cls) tex(endnotes.sty) tex(etoolbox.sty) tex(footmisc.sty) tex(nowidow.sty) tex(setspace.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
