...
RPROVIDES:${PN} += "perl(Pedigree) perl(Pedigree::AbortionNode) perl(Pedigree::Area) perl(Pedigree::ChildlessNode) perl(Pedigree::Language) perl(Pedigree::MarriageNode) perl(Pedigree::Node) perl(Pedigree::Parser) perl(Pedigree::PersonNode) perl(Pedigree::TwinsNode) texlive-pedigree-perl"
RDEPENDS:${PN} += "/bin/sh /usr/bin/perl coreutils ed findutils grep perl(FileHandle) perl(Getopt::Std) perl(strict) perl(vars) sed texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-pedigree-perl-bin texlive-scripts texlive-scripts-bin"
