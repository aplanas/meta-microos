...
RPROVIDES:${PN} += "tex(english-philosophy.lbx) tex(french-philosophy.lbx) tex(italian-philosophy.lbx) tex(philosophy-classic.bbx) tex(philosophy-classic.cbx) tex(philosophy-modern.bbx) tex(philosophy-modern.cbx) tex(philosophy-standard.bbx) tex(philosophy-verbose.bbx) tex(philosophy-verbose.cbx) tex(spanish-philosophy.lbx) texlive-biblatex-philosophy"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(authortitle.bbx) tex(authoryear-comp.bbx) tex(authoryear-comp.cbx) tex(standard.bbx) tex(verbose-trad2.cbx) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
