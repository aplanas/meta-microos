...
RPROVIDES:${PN} += "tex(title-DHBW.def) tex(title-JT-Aufsaetze.def) tex(title-JT-Geschichte.def) tex(title-JT-Typography.def) tex(title-KIT.def) tex(title-KOMAScript.def) tex(title-Markus-1.def) tex(title-Markus-2.def) tex(title-Spacer.def) tex(title-TU-DD.def) tex(title-TU-HH.def) tex(title-UKoLa.def) tex(title-WWUM.def) tex(uni-titlepage.sty) texlive-uni-titlepage"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(graphicx.sty) tex(scrbase.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
