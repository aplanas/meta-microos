...
RPROVIDES:${PN} += "typelib(XApp) typelib-1_0-XApp-1_0 typelib-1_0-XApp-1_0(aarch-64)"
RDEPENDS:${PN} += "libxapp.so.1()(64bit) python(abi) typelib(Atk) typelib(GLib) typelib(GModule) typelib(GObject) typelib(Gdk) typelib(GdkPixbuf) typelib(Gio) typelib(Gtk) typelib(HarfBuzz) typelib(Pango) typelib(cairo) typelib(freetype2) typelib(xlib)"
