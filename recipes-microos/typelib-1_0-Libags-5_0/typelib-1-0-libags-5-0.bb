...
RPROVIDES:${PN} += "typelib(Ags) typelib(AgsAudio) typelib(AgsGui) typelib-1_0-Libags-5_0 typelib-1_0-Libags-5_0(aarch-64)"
RDEPENDS:${PN} += "libags.so.5()(64bit) libags_audio.so.5()(64bit) libags_gui.so.5()(64bit) libags_server.so.5()(64bit) libags_thread.so.5()(64bit) typelib(GLib) typelib(GModule) typelib(GObject) typelib(Gdk) typelib(GdkPixbuf) typelib(Gio) typelib(Graphene) typelib(Gsk) typelib(Gst) typelib(GstAudio) typelib(GstBase) typelib(Gtk) typelib(HarfBuzz) typelib(Pango) typelib(PangoCairo) typelib(Soup) typelib(cairo) typelib(freetype2) typelib(libxml2)"
