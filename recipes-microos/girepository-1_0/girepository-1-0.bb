...
RPROVIDES:${PN} += "girepository-1_0 girepository-1_0(aarch-64) typelib(DBus) typelib(DBusGLib) typelib(GIRepository) typelib(GL) typelib(GLib) typelib(GModule) typelib(GObject) typelib(Gio) typelib(Vulkan) typelib(cairo) typelib(fontconfig) typelib(freetype2) typelib(libxml2) typelib(win32) typelib(xfixes) typelib(xft) typelib(xlib) typelib(xrandr)"
