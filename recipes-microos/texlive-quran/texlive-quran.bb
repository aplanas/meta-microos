...
RPROVIDES:${PN} += "tex(quran-transde.def) tex(quran-transen.def) tex(quran-transfa.def) tex(quran-transfr.def) tex(quran-translt.def) tex(quran.sty) tex(qurantext-de.translation.def) tex(qurantext-en.translation.def) tex(qurantext-en.transliteration.def) tex(qurantext-fa.translation.def) tex(qurantext-fr.translation.def) tex(qurantext-simple.def) tex(qurantext-uthmani-min.def) tex(qurantext-uthmani.def) texlive-quran"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(biditools.sty) tex(ifxetex.sty) tex(listofitems.sty) tex(xkeyval.sty) tex(xparse.sty) tex(xspace.sty) tex(xstring.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
