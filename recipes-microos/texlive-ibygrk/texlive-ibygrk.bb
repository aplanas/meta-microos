...
RPROVIDES:${PN} += "tex(IbycusHTG.enc) tex(Uibycus.fd) tex(Uibycus4.fd) tex(fibb84.tfm) tex(fibb848.tfm) tex(fibb849.tfm) tex(fibo84.tfm) tex(fibo848.tfm) tex(fibo849.tfm) tex(fibr84.tfm) tex(fibr848.tfm) tex(fibr849.tfm) tex(iby.map) tex(iby4extr.tex) tex(ibycus4.map) tex(ibycus4.sty) tex(ibycus4.tex) tex(ibycusps.tex) tex(psibycus.sty) tex(pssetiby.tex) tex(setiby4.tex) tex(tlgsqq.tex) tex(version4.tex) texlive-ibygrk"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(updmap.cfg) texlive texlive-filesystem texlive-ibygrk-fonts texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
