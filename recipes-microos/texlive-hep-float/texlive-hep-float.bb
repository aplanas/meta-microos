...
RPROVIDES:${PN} += "tex(hep-float.sty) texlive-hep-float"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(booktabs.sty) tex(calc.sty) tex(etoolbox.sty) tex(graphicx.sty) tex(kvoptions.sty) tex(multirow.sty) tex(subcaption.sty) tex(xparse.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
