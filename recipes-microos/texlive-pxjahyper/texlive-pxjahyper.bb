...
RPROVIDES:${PN} += "tex(pxjahyper-ajm.def) tex(pxjahyper-enc.sty) tex(pxjahyper-uni.def) tex(pxjahyper.sty) texlive-pxjahyper"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(atbegshi.sty) tex(bxjatoucs.sty) tex(etoolbox.sty) tex(keyval.sty) tex(ltxcmds.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
