...
RPROVIDES:${PN} += "tex(fontools_cs.enc) tex(fontools_l7x.enc) tex(fontools_lgr.enc) tex(fontools_ly1.enc) tex(fontools_oml.enc) tex(fontools_ot1.enc) tex(fontools_qx.enc) tex(fontools_t1.enc) tex(fontools_t2a.enc) tex(fontools_t2b.enc) tex(fontools_t2c.enc) tex(fontools_t3.enc) tex(fontools_t4.enc) tex(fontools_t5.enc) tex(fontools_ts1.enc) tex(fontools_ts3.enc) texlive-fontools"
RDEPENDS:${PN} += "/bin/sh /usr/bin/perl coreutils ed findutils grep perl(Cwd) perl(File::Basename) perl(File::Path) perl(File::Spec) perl(Getopt::Long) perl(List::Util) perl(POSIX) perl(Pod::Usage) perl(integer) perl(strict) perl(warnings) sed texlive texlive-filesystem texlive-fontools-bin texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
