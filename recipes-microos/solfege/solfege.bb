...
RPROVIDES:${PN} += "application() application(solfege.desktop) config(solfege) solfege solfege(aarch-64)"
RDEPENDS:${PN} += "/bin/bash /bin/sh /usr/bin/python3 lilypond-fonts-common python3-gobject-Gdk timidity typelib(GLib) typelib(GObject) typelib(Gdk) typelib(GdkPixbuf) typelib(Gtk) typelib(Pango)"
