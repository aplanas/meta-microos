...
RPROVIDES:${PN} += "asar nodejs-asar npm(@electron/asar) own(balanced-match) own(brace-expansion) own(chromium-pickle-js) own(commander) own(concat-map) own(fs.realpath) own(glob) own(inflight) own(inherits) own(minimatch) own(once) own(path-is-absolute) own(wrappy)"
RDEPENDS:${PN} += "/usr/bin/node nodejs(engine)"
