...
RPROVIDES:${PN} += "tex(elements.sty) tex(elements_names_brazil.def) tex(elements_names_english.def) tex(elements_names_french.def) tex(elements_names_german.def) tex(elements_names_portuges.def) tex(elements_names_spanish.def) texlive-elements"
RDEPENDS:${PN} += "/bin/sh coreutils ed findutils grep sed tex(etoolbox.sty) tex(translations.sty) texlive texlive-filesystem texlive-kpathsea texlive-kpathsea-bin texlive-scripts texlive-scripts-bin"
